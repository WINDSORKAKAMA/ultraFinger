//#ifdef INCLUDE 
  ...
  typedef enum{
      ...
      FINGERPRINT_DATA_CONFIRMATION_PACKET_RESPONSE_FAIL = 0x0E,        //! Module failed to receive the following data packages
      FINGERPRINT_DATA_CONFIRMATION_TEMPLATE_UPLOAD_FAIL = 0x0F,        //! Error when uploading image
  ...
  */
  ...
  #define FINGERPRINT_PACKET_SIZE_MINIMUM sizeof(struct Fingerprint_Packet_t) - (sizeof(uint8_t*) + sizeof(uint8_t))
    ...
    #define RECEIVE_ACK_PACKET(packet_type, data_length, data_pointer) \
      ...
      struct Fingerprint_Packet_t packet = packetReceive()
  ...
  extern "C"{    
    ...
    static struct Fingerprint_Packet_t packetReceive(){
      ...
      rxPacket.start_code = (((uint16_t)mySerial->read() << 8) | mySerial->read());
      ...
      rxPacket.length = (((uint16_t)mySerial->read() << 8) | mySerial->read());
      
      // data_buffer stores the data received
      uint8_t data_buffer[rxPacket.length - sizeof(rxPacket.sum)]; 
      ...
      rxPacket.sum = (((uint16_t)mySerial->read() << 8) | mySerial->read());
    ...
    static bool handleReceivedData(uint8_t data_code, uint8_t* keywords){
      ...
      switch(data_code){
        ...
        case FINGERPRINT_DATA_CONFIRMATION_IMAGE_DISORDERLY: 
          Serial.println("Error! Fingerprint Is Overly Diorderly.\n");
          break;
          return 0;

        case FINGERPRINT_DATA_CONFIRMATION_FEATURE_FAIL:
          Serial.println("Error! Inufficient Fingerprint Data Collected.\n");
          break;
          return 0;
    ...
    void passwordVerify(uint8_t password[4]){
      ...
      // handle the data in the acknowledge packet
      handleReceivedData(*packet.data, "Correct Password.");
    }
    ...
    void passwordSet(uint8_t new_password[4]){
      ...
      if(handleReceivedData(*packet.data, "Password Setting Complete.")){
    ...
    void setModuleAddress(uint8_t new_address[4]){
      ...
      if(handleReceivedData(*packet.data, "Address Setting Complete.")){
    ...
    void systemBasicParameterSet(uint8_t parameter_number, uint8_t new_parameter){
      ...
      // handle the data in the acknowledge packet
      if(handleReceivedData(*packet.data, "Parameter Setting Complete."))
    ...
    void controlUARTPort(uint8_t port_state){
      ...
      // handle the data
      handleReceivedData(*packet.data, "Port Operation Complete.");
    }
    ...
    struct Fingerprint_Helper_t systemParameterRead(){
      ...
      // assign the data
      if(handleReceivedData(*packet.data, "Parameter Read Complete.")){
        fingerprint_module.system_parameter_store.status_register = ((uint16_t)packet.data[0] << 8) | packet.data[1];
        fingerprint_module.system_parameter_store.system_ID_code = ((uint16_t)packet.data[2] << 8) | packet.data[3];
        fingerprint_module.system_parameter_store.finger_lib_size = ((uint16_t)packet.data[4] << 8) | packet.data[5];
        fingerprint_module.system_parameter_store.security_level = ((uint16_t)packet.data[6] << 8) | packet.data[7];

        fingerprint_module.system_parameter_store.device_address[0] = packet.data[8];
        fingerprint_module.system_parameter_store.device_address[1] = packet.data[9];
        fingerprint_module.system_parameter_store.device_address[2] = packet.data[10];
        fingerprint_module.system_parameter_store.device_address[3] = packet.data[11];

        print_module.system_parameter_store.data_packet_size = ((uint16_t)packet.data[12] << 8) | packet.data[13];
        fingerprint_module.system_parameter_store.baud_setting = ((uint16_t)packet.data[14] << 8) | packet.data[15];
      }
    ...
    // The 1 byte is for the instruction code
      ...
      if(handleReceivedData(*packet.data, "Number Generation Success."))
        // return the template number
        return ((uint16_t)packet.data[1] << 8) | packet.data [2];
    }
    ...
    void fingerprintVerify(uint8_t capture_time, uint16_t start_bit_number,
      ...
      */
      ...
      if(handleReceivedData(*packet.data, "Fingerprint Read Complete.")){
        result = uint8_t result_buff[4];
                              
        result[0] = (uint16_t)packet.data[1] << 8;
        result[0] = packet.data[2];

        result[1] = (uint16_t)packet.data[3] << 8;
        result[1] = packet.data[4];
      }
    }
    ...
    uint32_t generateRandomCode(){
      ...
      if (handleReceivedData(*packet.data, "Number Generation Success."))
        ...
        return (uint32_t)packet.data[1] << 24 | (uint32_t)(packet.data[2] << 16) | (uint32_t)packet.data[3] << 8 | packet.data[4];
    }
  };  
// #endif
